...
export default function Topbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md warm-shadow' : 'bg-transparent'
        }`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link
              href="/"
              className={`font-medium text-2xl transition-all duration-300 ${isScrolled ? 'text-coffee hover:text-dark-coffee' : 'text-white hover:text-warm-white'
                }`}
            >
              Kevin Sutandi
            </Link>
      ...
      <div className={`fixed inset-0 z-40 transition-all duration-300 ${isOpen ? 'visible opacity-100' : 'invisible opacity-0'
        ...
        <div className="absolute inset-0 bg-coffee/95 backdrop-blur-md"></div>
        <div className="relative h-full flex flex-col justify-center items-center space-y-8 px-4">
          <Link
            href="https://gallery.kevinsutandi.com"
            onClick={() => setIsOpen(false)}
            className={`text-warm-white text-2xl font-light hover:text-soft-gold transition-all duration-200 tracking-wider transform ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
              }`}
